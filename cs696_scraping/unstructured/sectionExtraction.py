...
class SectionExtraction:
    ...
    def extractHTMLSections(self, raw_10k):
        ...
        matches = regex.finditer(self.document['10-K'])
        ...
        print("Sections Extracted:{}".format(list(self.pos_dat.index)))
    #     print("send any section name as printed")
        return list(self.pos_dat.index)

    def extractTextFromSection(self, key, document_type = "10-K"):
        
        if key in list(self.pos_dat.index):
            if document_type not in self.document:
                ...
                if len(end) == "0":
                    ...
                    item_raw = self.document['10-K'][self.pos_dat['start'].loc[key]:self.pos_dat['end'].loc[end]]
